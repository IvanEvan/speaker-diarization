...
def vggvox_resnet2d_icassp(input_dim=(257, 250, 1), num_class=8631, mode='train', args=None):
    ...
    if mode == 'eval':
        y = keras.layers.Lambda(lambda x: keras.backend.l2_normalize(x, axis=1))(x)

    model = keras.models.Model(inputs=inputs, outputs=y, name='vggvox_resnet2D_{}_{}'.format(loss, aggregation))
